...
(function() {
    ...
    async function _formatMarkdownForKeep(suppressError) {
        ...
        const plainContentDoms = document.querySelectorAll(`[contenteditable="true"][spellcheck="true"]`);
        const origTitleDom = plainContentDoms[0];
        const origContentDom = plainContentDoms[1];
        const listItemsDoms = document.querySelectorAll(`[contenteditable="true"][aria-label="list item"]`);
        let shouldBindData = listItemsDoms.length > 0 || plainContentDoms.length > 0;
        if (!contentDom) {
        ...
        if (shouldBindData) {
            ...
            if (document.querySelectorAll(`[contenteditable="true"][aria-label="list item"]`).length > 0) {
                ...
                // is a plain text
                newContentHtml = origContentDom.innerText.replace(/ /g, `&nbsp;`).replace(/\n/g, "<br />");
            }
            // append the content
            ...
            if (newContentHtml !== articleContent.innerHTML && document.activeElement.id !== "content-dialog") {
                ...
                try {
                    ...
                    try {
                        noteImages = [...origTitleDom.parentElement.parentElement.parentElement.querySelectorAll('img')].reverse();
                    } catch (err) {}
                }
