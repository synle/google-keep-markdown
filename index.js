  ...
  async function _formatMarkdownForKeep(suppressError) {
    ...
    if (shouldBindData) {      
      ...
      if (
        ...
        try {
          noteImages = [
            ...origTitleDom.previousSibling.querySelectorAll("img"),
          ].reverse();
        } catch (err) {
          try {
            noteImages = [
              ...origTitleDom.parentElement.parentElement.parentElement.querySelectorAll(
                "img"
              ),
            ].reverse();
          } catch (err) {}
        ...
        if (noteImages.length > 0) {
          ...
          const btnChangeImageListOrder = figureImages.querySelector(
            "#btnChangeImageListOrder"
          );
